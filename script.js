...
function changeImage(direction, productNumber) {
  if (direction === "left") {
    currentImageIndex[productNumber] =
      currentImageIndex[productNumber] > 0
        ? currentImageIndex[productNumber] - 1
        : productImages[productNumber].length - 1;
  } else if (direction === "right") {
    currentImageIndex[productNumber] =
      currentImageIndex[productNumber] < productImages[productNumber].length - 1
        ? currentImageIndex[productNumber] + 1
        : 0;
  }

  // Update the image src based on the current index
  document.getElementById(`productImage${productNumber}`).src =
    productImages[productNumber][currentImageIndex[productNumber]];
}
